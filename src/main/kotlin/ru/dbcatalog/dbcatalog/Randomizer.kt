...
class Randomizer {
    ...
    fun addMusic(): Int {
        ...
        val musicId = db.queryWithResult(db.getConnect().prepareStatement(query).apply {
            setString(1, name)
            setInt(2, musicYear)
            setInt(3, duration)
        })[0]["id"] as Int
        ...
        if (musicAlbumExist) {
            //language=PostgreSQL
            query = "INSERT INTO db.music_has_album (music_id, music_album_id) VALUES (?, ?);"
            db.queryWithResult(db.getConnect().prepareStatement(query).apply {
                setInt(1, musicId)
    ...
    private fun randomArtist(type: Int): Int {//0 - соло, 1 - группаи
        ...
        return db.queryWithResult(statement)[0]["id"] as Int
    }
    ...
    fun addFilm(): Int {
        ...
        val filmId = db.queryWithResult(db.getConnect().prepareStatement(query).apply {
            setString(1, name)
            setInt(2, filmYear)
            setInt(3, duration)
            setString(4, desc)
        ...
        if (musicExist) {
            ...
            while (numOfMusic != 0) {
                ...
                //language=PostgreSQL
                query = "INSERT INTO db.film_has_music (film_id, music_id) VALUES (?, ?);"
                db.queryWithResult(db.getConnect().prepareStatement(query).apply {
                    setInt(1, filmId)
                    setInt(2, musicId)
                })
        ...
        for (peopleIdInGroup in group) {
            ...
            //language=PostgreSQL
            query = "INSERT INTO db.film_has_people (film_id, people_id, people_function_id) VALUES (?, ?, ?);"
            db.queryWithResult(db.getConnect().prepareStatement(query).apply {
                setInt(1, filmId)
        ...
        for (i in 0 until genreNum) {
            ...
            //language=PostgreSQL
            query = "INSERT INTO db.film_has_film_genre (film_id, film_genre_id) VALUES (?, ?);"
            db.queryWithResult(db.getConnect().prepareStatement(query).apply {
                setInt(1, filmId)
                setInt(2, genreId)
            })
    ...
    fun addBook(): Int {     // возврат - id книги
        ...
        val bookId = db.queryWithResult(statement)[0]["id"] as Int    // id добавленной книги
    ...
    fun addTop(type: Int = Random.nextInt(0, 3)) {
        ...
        val queryForInsertInoCross: String  // query для записи в top_has_film / top_has_music / top_has_book
        ...
        when (type) {
            0 -> {
                ...
                //language=PostgreSQL
                queryForInsertInoCross = "INSERT INTO db.top_has_film (top_id, film_id, position) VALUES (?, ?, ?);"
            }
            1 -> {
                noun = "песен"
                //language=PostgreSQL
                ...
                //language=PostgreSQL
                queryForInsertInoCross = "INSERT INTO db.top_has_music (top_id, music_id, position) VALUES (?, ?, ?);"
            }
            else -> {
                noun = "книг"
                //language=PostgreSQL
                ...
                //language=PostgreSQL
                queryForInsertInoCross = "INSERT INTO db.top_has_book (top_id, book_id, position) VALUES (?, ?, ?);"
            }
        }
        ...
        val topId =
            db.queryWithResult(db.getConnect().prepareStatement("INSERT INTO db.top (name) VALUES (?) RETURNING id;")
        ...
        while (num != 0) {
            ...
            db.queryWithResult(
                db.getConnect()
                    .prepareStatement(queryForInsertInoCross)
                    .apply {
                        setInt(1, topId)
                        setInt(2, elId)
                        setInt(3, num)
    ...
    fun addUser(): Int {
        ...
        val userId = db.queryWithResult(statementForAddUserEntry)[0]["id"] as Int
    ...
    private fun generateUsersViewedContent(userId: Int, contentType: ContentType) {
        ...
        for (i in 1..Random.nextInt(4, minOf(20, listOfContentIds.size))) {
            ...
            db.queryWithResult(db.getConnect().prepareStatement(insertQuery).apply {
                setInt(1, userId)
                setInt(2, randomId)
    ...
    private fun fillGenre(contentType: ContentType) {
        ...
        for (genre in genreList) {
            ...
            val statement = db.getConnect().prepareStatement(queryForInsert).apply {
